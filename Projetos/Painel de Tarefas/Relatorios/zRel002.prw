...
Faz a impressão do relatório zRel002
@author CÁRITA DIAS MARTINS DA SILVA
@since 04/04/2025
@version 1.0
@type function
...
Static Function fImprime()
    ...
    Local nAtuAux      := 0
    Local cQryAux      := ''
    Local cArquivo     := 'zRel002'+RetCodUsr()+'_' + dToS(Date()) + '_' + StrTran(Time(), ':', '-') + '.pdf'
    Local cStatus      := ''
    Local cDataIni
    Local cDataFin
    ...
    Private nColDad3    := nColIni + 300 // 1 - tava 100 mudei pra 250 // 300
    Private nColDad4    := nColIni + 420 // 2-tava 200 mudei pra 350 3-aumentei 50
    Private nColDad5    := nColIni + 500 // 2-tava 250 mudei pra 400 3-aumentei 50
    Private nColDad6    := nColIni + 450 // 3-aumentei 50 4-aumentei 50
    ...
    If ! QRY_AUX->(EoF())
        ...
        While ! QRY_AUX->(EoF())
            ...
            //Faz o zebrado ao fundo dps atribui a cor de acordo com o Status
            If nAtuAux % 2 == 0
            ...
            oPrintPvt:SayAlign(nLinAtu, nColDad2, Alltrim(QRY_AUX->ZE1_TITULO), oFontDet, 150, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
            oPrintPvt:SayAlign(nLinAtu, nColDad3, Alltrim(cStatus), oFontDet, 100, 10, nClrText0, PAD_LEFT, /*nAlignVert*/)
            oPrintPvt:SayAlign(nLinAtu, nColDad4, dToC(QRY_AUX->ZE1_DT), oFontDet, 50, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
            oPrintPvt:SayAlign(nLinAtu, nColDad5, dToC(QRY_AUX->ZE1_DTFINA), oFontDet, 50, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
...
Static Function fImpCab()
    ...
    If nPagAtu == 1
        ...
        oPrintPvt:SayAlign(nLinAtu, nColIni, 'Desde de: ', oFontDetN, 200, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
        oPrintPvt:SayAlign(nLinAtu, nColIni+200, cTexto, oFontDet, 200, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
        nLinAtu += 15
        
        cTexto := dToC(MV_PAR02)
        oPrintPvt:SayAlign(nLinAtu, nColIni, 'Até: ', oFontDetN, 200, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
        oPrintPvt:SayAlign(nLinAtu, nColIni+200, cTexto, oFontDet, 200, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
        nLinAtu += 15
    ...
    oPrintPvt:SayAlign(nLinAtu, nColDad3, 'Status', oFontMin, 100, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
    oPrintPvt:SayAlign(nLinAtu, nColDad4, 'Iniciado em', oFontMin, 50, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
    oPrintPvt:SayAlign(nLinAtu, nColDad5, 'Finalizado em', oFontMin, 50, 10, /*nClrText*/, PAD_LEFT, /*nAlignVert*/)
    nLinAtu += 15
Return
...
Static Function fImpRod()
    ...
    //Dados da Esquerda
    cTexto := dToC(dDataBase) + '     ' + cHoraEx + '     ' + FunName() + ' (zRel002)     ' + UsrRetName(RetCodUsr())
